...
status = cycle(['made by mohhmad', "powerful server security provided by allah", "nuker ki mkc", 'only allah can nuke server '])
...
async def help(ctx):
    embed=discord.Embed(
        title="**Storm | Help Panel**",
        description=f"**Use `{client.command_prefix}help <module> or <command>` to extend information of a command**",
        color = 0x050505
    )
    embed.set_thumbnail(url=ctx.author.avatar_url)
    embed.set_footer(text=f"Developed by Venom | Requested by {ctx.author.name}", icon_url=ctx.author.avatar_url)
    embed.add_field(name="__**Modules**__", value = "Information \n Moderation \n Antinuke", inline=False)
    embed.add_field(name="Links", value = "[Support Server](https://discord.gg/srw)", inline=False)
...
async def on_webhook_update(webhook):
  reason = "Storm | Anti Webhook Create"
  guild = webhook.guild
...
async def on_webhooks_update(webhook):
  reason = "Storm | Anti Webhook Delete"
  guild = webhook.guild
...
@client.event
async def on_webhook_update(webhook):
    reason="Storm | Anti Webhook Update"
    guild = webhook.guild
